...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "openGlobalKeybindings":
			vscode.commands.executeCommand("workbench.action.openGlobalKeybindings", message.text ?? "mm-code.")
			break
		...
		case "ghostServiceSettings":
			...
			await provider.postStateToWebview()
			vscode.commands.executeCommand("mm-code.autocomplete.reload")
			break
		...
		case "saveApiConfiguration":
			if (message.text && message.apiConfiguration) {
				try {
					...
					await updateGlobalState("listApiConfigMeta", listApiConfig)
					vscode.commands.executeCommand("mm-code.autocomplete.reload") // kilocode_change: Reload autocomplete model when API provider settings change
				} catch (error) {
					provider.log(
